...
function updateBottomTables() {
    ...
    /* Lastly, add the button to submit, although now it prompts the guest to login. */
                        ...
                        if ((new FormData(topForm)).get("labForm") == r.labSeat.lab) {
                            ...
                            if (r.isAnonymous) {
                                ...
                                if (r.walkInStudent == null) {
                                    document.getElementById("reserver").innerHTML = `<a href = temp_guest_search_profile.html?username=${r.email}>${r.email}</a>`;
                                } else {
